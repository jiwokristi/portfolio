...
export function CsFeatures({ features }: { features: ProjectFeature[] }) {
  ...
  return (
    <section ref={containerRef} className="bg-bg-secondary py-section">
      <Container>
        <h2 className="mb-12 text-center text-h2 font-bold">Key Features</h2>
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {features.map(feature => (
            ...
            >
              <h3 className="mb-2 text-h3 font-semibold transition-colors group-hover:text-accent">{feature.title}</h3>
              <p className="mb-5 text-sm text-text-secondary leading-relaxed">{feature.description}</p>
              {feature.tags && (
                <div className="flex flex-wrap gap-1.5">
                  {feature.tags.map(tag => (
                    <Badge key={tag} variant="contained" color="subtle" size="sm">
                      {tag}
                    </Badge>
                  ))}
                </div>
